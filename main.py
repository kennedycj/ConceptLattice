...
if __name__ == '__main__':
    ...
    index_date = date(1900, 1, 1)
    for this_year in range(project_start.year, project_end.year + 1):
        start_month = 1
        end_month = 13
        if this_year == project_start.year:
            start_month = project_start.month

        print(f"this year = {this_year}")
        if this_year == project_end.year:
            end_month = project_end.month

        for this_month in range(start_month, end_month):
            month_start = len(days)
            for year, month, date, weekday in cal.itermonthdays4(this_year, this_month):
                if year < this_year:
                    continue
                day = Reference(day_abbr[weekday][0], len(days), len(days) + 1, date, ReferenceType.DAY)
                days.append(day)

            months.append(Reference(month_abbr[this_month], month_start, len(days), len(months), ReferenceType.MONTH))

    for day in days:
        print(f"{day.start} {day.end} {day.name}")

    for month in months:
        print(f"{month.id} {month.start} {month.end} {month.name}")

    #for year, month, day, weekday in cal.itermonthdays4(project_start.year, project_start.month):
        #n_days += 1
        #print(f"year = {year} month = {month_abbr[month]} day = {day} weekday = {day_abbr[weekday][0]}")
    ...
    #year_range = ws.range('A1')
    ws.range('A1').value = project_start
    ws.range('A1').autofill(ws.range('A1:D1'), type_='fill_years')
    #print(f"VALUE = {year_range[0].value}") # = project_start
    #event_range = year_range.autofill(ws.range(f"A1:A{3}"), type_='fill_years')
